...
function SearchBar({ fetchJobsCustom, jobCriteria, setJobCriteria }) {
    ...
    const search = async () => {
        const hasCriteria = Object.values(jobCriteria).some(value => value !== "");
        if (hasCriteria) {
    ...
    const clearFilters = () => {
        const resetCriteria = { title: '', location: '', experience: '', type: '' };
        setJobCriteria(resetCriteria);
        fetchJobsCustom(resetCriteria); // Reset search
    };
    ...
    return (
        <div className='flex gap-4 my-10 justify-center px-10'>
            ...
            <button onClick={clearFilters} className='w-64 bg-red-500 text-white font-bold py-3 rounded-md'>
                Clear Filters
            </button>
        </div>
    );
